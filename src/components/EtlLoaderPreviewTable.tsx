...
export const EtlLoaderPreviewTable: React.FC<EtlLoaderPreviewTableProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* SUMMARY STATS & FORMAT BAR */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="p-3 bg-white border border-gray-200 rounded-lg shadow-2xs">
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
            Total Exploded ETL Records
          </div>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-xl font-extrabold text-gray-900 font-mono">{records.length}</span>
            <span className="text-xs text-gray-500">GL Ledger Rows</span>
          </div>
          <div className="text-[10px] text-gray-500 mt-0.5">
            {multiSplitCount} line-item splits calculated
          </div>
        </div>

        <div className="p-3 bg-white border border-gray-200 rounded-lg shadow-2xs">
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
            Total Batch Value (USD Equiv)
          </div>
          <div className="flex items-baseline gap-2 mt-1">
          ...
          <div className="text-[10px] text-emerald-600 font-medium mt-0.5 flex items-center gap-1">
            ...
            100% Apportionment Balance Checked
          </div>
        </div>

        <div className={`p-3 rounded-lg border shadow-2xs ${
          totalErrorCount > 0 ? 'bg-amber-50/70 border-amber-200' : 'bg-emerald-50/70 border-emerald-200'
        }`}>
          <div className="text-[11px] font-semibold text-gray-700 uppercase tracking-wider flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className={`text-xl font-extrabold font-mono ${
              totalErrorCount > 0 ? 'text-amber-800' : 'text-emerald-800'
            }`}>
              {totalErrorCount > 0 ? `${totalErrorCount} Errors` : 'All Mapped ✓'}
            </span>
          </div>
          <div className="mt-1">
            {totalErrorCount > 0 ? (
              ...
              >
                Resolve Missing Mappings &rarr;
              </button>
            ) : (
              <span className="text-[10px] text-emerald-700">Yardi Vendor & Property IDs Verified</span>
            )}
          </div>
        </div>

        <div className="p-3 bg-white border border-gray-200 rounded-lg shadow-2xs flex flex-col justify-between">
          <div>
            <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
              Loader Schema Target
            </div>
            <select
              value={format}
              onChange={e => onFormatChange(e.target.value as InvoiceETLFormat)}
              className="mt-1 w-full px-2 py-1 bg-gray-50 border border-gray-300 rounded text-xs font-bold text-[#EA580C] focus:ring-1 focus:ring-[#EA580C] outline-hidden cursor-pointer"
            >
              <option value="YARDI_VOYAGER_LOADER">Yardi Voyager / PayScan (Granular GL)</option>
              <option value="NETSUITE_INVOICE_SYNC">Oracle NetSuite Multi-Line Journal</option>
              <option value="SAP_AR_AP_FEED">SAP S/4HANA AR/AP Feed</option>
              <option value="JSON_INVOICE_STREAM">JSON Stream (Developer Payload)</option>
              <option value="XML_PEPPOL_UBL">PEPPOL / UBL XML 2.1 Standard</option>
            </select>
          </div>
          <div className="flex items-center justify-between text-[10px] text-gray-500 mt-2">
            <span>Selective fields editable inline</span>
            <button
              onClick={onResetOverrides}
              className="text-gray-400 hover:text-gray-700 flex items-center gap-1 cursor-pointer"
              title="Reset all user-edited notes and codes to system defaults"
            >
